...
[ [Learn more about OWL2](https://www.w3.org/TR/2012/REC-owl2-primer-20121211/){:target="blank"} ]
 [ [Learn more about ontology relations in GO](http://localhost:4000/docs/ontology-relations/){:target="blank"} ]
...
## Federated Queries
Lastly, Federated Queries. Those specific types of queries are designed to retrieve specific pieces of information from multiple SPARQL endpoint (e.g. sparql.geneontology.org, sparql.uniprot.org, sparql.wikipathway.org, etc). The whole concept of federated queries rely on the selection and sharing of unique IRIs to describe each entity. If a gene is universally identified by an IRI, then it is possible to request data across multiple databases in a single query. This technique, albeit powerful can however suffer from latency and is vulnerable if any of the endpoint is unavailable. In addition, it's critical to ensure you are gathering data about the same entity through a unique IRI or an IRI properly mapped between resources. [ [Learn more about Federated Queries](https://www.w3.org/TR/sparql11-federated-query/){:target="blank"} ].

> This was a primer in SPARQL but there are many other features not described here. We encourage users of the SPARQL endpoint to learn more about SPARQL [here](https://www.w3.org/TR/sparql11-overview/){:target="blank"} and to contact us with questions related to the GO SPARQL endpoint on the [GO Helpdesk](http://help.geneontology.org/){:target="blank"}
